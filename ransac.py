...
def construct_overdetermined_equation(params, similar_rvecs, similar_tvecs, outlier_points, outlier_points1):
    p1 = params[:4]
    delta_d = params[4:]
    q1 = outlier_points
    ...
    residual = 0.5 * np.linalg.norm(np.dot(p1, p1.T) + 0.5 *np.dot(p1 + delta_d, p1 + delta_d).T - 0.5 * 2 * np.dot(p1.T, p1 + delta_d) * (np.dot(q1, q3) / (np.linalg.norm(q1) * np.linalg.norm(q3))) - 0.5 * np.dot(p1 - transform[:, 0], p1 - transform[:, 0]).T + 0.5 *np.dot(p1 + delta_d - transform[:, 0], p1 + delta_d - transform[:, 0]).T - 0.5 * 2 * np.dot(p1 - transform[:, 0].T, p1 +delta_d - transform[:, 0]) * (np.dot(q4, q2) / (np.linalg.norm(q4) * np.linalg.norm(q2))))
...
for i in range(len(png_files) - 1):
    ...
    image1 = cv2.imread(png_files[i])
    image2 = cv2.imread(png_files[i + 1])
    # 读取两幅图像
    ...
    triangulated_points_out = triangulate_points(keypoints_outside1, keypoints_outside2, initial_rvec, initial_tvec)
    ...
    num_samples = len(keypoint_coordinates1)  # Number of point pairs
    all_solution_p1 = []  # Initialize a list to store all solution_p1
    all_solution_delta_d = []
    all_solution_p2 = []
    for i in range(num_samples):
        point1 = keypoint_coordinates1[i]
        point2 = keypoint_coordinates2[i]
        # 使用最小二乘法求解
        result = least_squares(construct_overdetermined_equation, initial_params, args=(similar_rvecs, similar_tvecs, point1, point2))
    ...
    # 合并两个列表
    combined_data = triangulated_points_out + all_solution_p1
    combined_data1 = triangulated_points_out + all_solution_p2
