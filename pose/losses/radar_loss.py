...
class EgoMotionLoss(Module):
    ...
    # def forward(self, pc1, pre_trans, tlio_trans, rotation_matrix, gt_trans):
    def forward(self, pc1, pre_trans, gt_trans):
        ...
        Return (torch.Tensor):
            total_loss: [1]
        """
        # noise = np.random.rand(4, 1)
        # gt_trans = gt_trans.cpu().numpy()
        # gt_trans[:, -1] += noise[:, 0]


        # initial_state = torch.eye(4, device=pre_trans.device)
        # initial_covariance = torch.eye(4, device=pre_trans.device)
        # measurement_noise_covariance = torch.eye(4, device=pre_trans.device) * 0.000000001
        #
        # fused_state, fused_covariance = self.state_update(initial_state, pre_trans, initial_covariance,
        #                                                   measurement_noise_covariance)
        # fused_state, fused_covariance = self.state_update(fused_state, tlio_trans, fused_covariance,
        #                                                   measurement_noise_covariance)
        # new_trans = fused_state
        # translation = new_trans[:, :3, 3]
        #
        # # Extract the rotation part from pre_trans
        # rotation = pre_trans[:, :3, :3]
        # batch_size = new_trans.size(0)
        # combined_transformation = torch.eye(4, device=new_trans.device).unsqueeze(0).repeat(batch_size, 1, 1)
        # combined_transformation[:, :3, :3] = rotation
        # combined_transformation[:, :3, 3] = translation
        # pc1_pre = torch.matmul(new_trans[:,:3,:3], pc1)+new_trans[:,:3,3].unsqueeze(2)
    ...
    def state_update(self, state, measurement, covariance, measurement_noise_covariance):
        # 预测步骤：更新状态和协方差
        ...
        kalman_gain = torch.matmul(predicted_covariance, torch.inverse(innovation_covariance))

        # 更新状态和协方差
        updated_state = predicted_state + torch.matmul(kalman_gain, innovation)
        updated_covariance = torch.matmul((torch.eye(4, device=state.device) - kalman_gain), predicted_covariance)
...
class RadarFlowLoss(Module):
    ...
    #                          mseg_gt=None, dyn_mask=None, radar_u=None, radar_v=None, opt=None, tlio_trans=None, rotation_matrix=None, basic_f=None):
    def forward(self, args, pc1, pc2, pred_f, vel1, gt_f=None, pre_trans=None, mseg_pre=None, gt_trans=None, \
        ...
        if args.model == 'raflow':
            ...
            total_loss = self.w_self * self_sup_loss+self.w_opt * opt_flow_loss

        if args.model == 'cmflow' or args.model == 'cmflow_t':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            # ego_motion_loss = self.ego_motion_loss(pc1, pre_trans, tlio_trans, rotation_matrix, gt_trans)
            ...
            items['opticalLoss'] = opt_flow_loss.item()
            items['motionLoss'] = dyn_flow_loss.item()
            ...
            items['totalLoss'] = total_loss.item()
        return total_loss, items
